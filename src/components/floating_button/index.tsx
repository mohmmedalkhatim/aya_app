...
function FloatingButton() {
    let open = useStore(item => item.medicationDialogState)
    let setOpen = useStore(item => item.setDialogState)
    ...
    return (
        <>
            ...
            <AnimatePresence>
                {open && (
                    <motion.dialog open={open} onClose={() => setOpen(true)} className="fixed inset-0 w-full top-20 backdrop-blur-sm flex items-center justify-center z-50" initial={{ opacity: 0, translateY: "2rem" }} animate={{ opacity: 1, translateY: "0rem" }} exit={{ opacity: 0 }}>
                        <Event_form onCancel={() =>
